...
// Form state type
interface GroupModuleFormState {
  groupType: {
    value: string;
    error: string;
  };
...
// Initial form state
const initialFormState = (preState?: any): GroupModuleFormState => ({
  groupType: {
...
const AddEditGroupModuleForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState<GroupModuleFormState>(
    initialFormState()
  );
  ...
  const breadcrumbs = [
    ...
    {
      label: strings.GROUP_MODULES,
      href: urls.groupsViewPath,
      icon: FiUsers,
    },
    {
      label: isEdit ? strings.EDIT_GROUP_MODULE : strings.ADD_GROUP_MODULE,
      isActive: true,
      icon: FiPlus,
    },
  ];
  ...
  useEffect(() => {
    if (isEdit && id) {
      ...
      if (state?.groupModuleData) {
        setFormData(initialFormState(state.groupModuleData));
      } else {
        loadGroupModule();
      }
    }
  ...
  const loadGroupModule = async () => {
    setLoading(true);
    try {
      const groupModule = await groupModuleServices.getById(id!);
      if (groupModule) {
        setFormData(initialFormState(groupModule));
      } else {
        navigate(urls.groupModuleViewPath);
      }
    } catch (error: any) {
      console.error("Error loading group module:", error);
      toast.error(error.message || "Failed to fetch group module");
      navigate(urls.groupModuleViewPath);
    } finally {
      setLoading(false);
    }
  ...
  const handleInputChange =
    (field: keyof GroupModuleFormState) =>
  ...
  const handleBlur = (field: keyof GroupModuleFormState) => () => {
  ...
  const validateForm = (): boolean => {
    const errors: Partial<GroupModuleFormState> = {};
    let isValid = true;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = isEdit
        ? await groupModuleServices.update(id!, groupModuleData)
        : await groupModuleServices.create(groupModuleData);
      ...
      console.error("Error saving group module:", error);
      toast.error(error.message || "Failed to save group module");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-theme-secondary">
      <ModuleHeader
        title={isEdit ? strings.EDIT_GROUP_MODULE : strings.ADD_GROUP_MODULE}
...
export default AddEditGroupModuleForm;
