...
const transformAccountToRow = (account: any): Row => {
  ...
  return {
    ...
    createdTime: new Date(account.createdAt).toISOString().split("T")[0],
    updatedTime: new Date(account.updatedAt).toISOString().split("T")[0],
    inactiveTime: new Date(account.updatedAt).toISOString().split("T")[0],
    // Store IDs for edit functionality
    parentAccount: typeof account.parentAccount === 'object' && account.parentAccount 
...
getAll: async (): Promise<Row[]> => {
  try {
    ...
    if (response.success) {
      ...
      accounts.push({
        ...
        children: response.data.children?.map((child:any) => child._id) || [],
  ...
  update: async (
    ...
    try {
      ...
      if (accountData.accountName !== undefined) payload.accountName = accountData.accountName;
      if (accountData.parentAccount !== undefined) payload.parentAccount = accountData.parentAccount;
      if (accountData.clientId !== undefined) payload.clientId = accountData.clientId;
  ...
  getClients: async (): Promise<Client[]> => {
    try {
      ...
      if (response.success) {
        return response.data.data.filter((client) => client.status === "active");
      } else {
...
getAccountHierarchy: async (): Promise<{ parentAccount: ParentAccountInfo | null; currentAccount: ParentAccountInfo | null }> => {
  try {
    ...
    if (response.success) {
      ...
      if (!response.data.parentAccount || Object.keys(response.data.parentAccount).length === 0) {
        currentAccount = {
          _id: response.data._id,
          accountName: response.data.accountName,
          level: response.data.level,
