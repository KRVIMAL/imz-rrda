...
const transformUserToRow = (user: UserData): Row => ({
  ...
  fullName: user.fullName || `${user.firstName} ${user.middleName} ${user.lastName}`.trim(),
  email: user.email,
  contactNo: user.contactNo,
  userRole: "User", // This will be populated from role API
  status: user.status,
  createdTime: new Date(user.createdAt).toISOString().split("T")[0],
  updatedTime: new Date(user.updatedAt).toISOString().split("T")[0],
  inactiveTime: new Date(user.updatedAt).toISOString().split("T")[0],
...
const flattenAccountHierarchy = (account: AccountHierarchyResponse, result: FlatAccount[] = []): FlatAccount[] => {
  // Add current account
  result.push({
    _id: account._id,
...
export const userServices = {
  ...
  update: async (
    ...
    try {
      ...
      if (userData.middleName !== undefined) payload.middleName = userData.middleName;
      if (userData.lastName !== undefined) payload.lastName = userData.lastName;
      if (userData.email !== undefined) payload.email = userData.email;
      if (userData.contactNo !== undefined) payload.contactNo = userData.contactNo;
