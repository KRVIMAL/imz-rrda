// src/modules/vehicles/services/vehicles.services.ts
...
const transformVehicleToRow = (vehicle: VehicleData): Row => ({
  ...
  createdTime: new Date(vehicle.createdAt).toISOString().split('T')[0],
  updatedTime: new Date(vehicle.updatedAt).toISOString().split('T')[0],
  inactiveTime:  new Date(vehicle.updatedAt).toISOString().split('T')[0],
});
...
export const vehicleServices = {
  ...
  update: async (
    ...
    try {
      ...
      if (vehicleData.brandName !== undefined) payload.brandName = vehicleData.brandName;
      if (vehicleData.modelName !== undefined) payload.modelName = vehicleData.modelName;
      if (vehicleData.vehicleType !== undefined) payload.vehicleType = vehicleData.vehicleType;
