...
const transformRoadMasterToRow = (roadData: RoadMasterData): Row => ({
  ...
  createdTime: new Date(roadData.createdAt).toISOString().split("T")[0],
  updatedTime: new Date(roadData.updatedAt).toISOString().split("T")[0],
  inactiveTime: new Date(roadData.updatedAt).toISOString().split("T")[0],
});
...
export const roadMasterServices = {
  ...
  // Export function
  exportData: async (format: "csv" | "pdf" | "excel" = "csv"): Promise<string> => {
    try {
