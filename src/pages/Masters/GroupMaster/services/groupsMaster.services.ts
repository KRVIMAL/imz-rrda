...
const transformGroupMasterToRow = (groupMaster: GroupMasterData): Row => {
  ...
  if (Array.isArray(groupMaster.groupModule) && groupMaster.groupModule.length > 0) {
    // Populated object
  ...
  if (Array.isArray(groupMaster.imei)) {
    if (groupMaster.imei.length > 0) {
      ...
      if (typeof groupMaster.imei[0] === 'object' && 'deviceIMEI' in groupMaster.imei[0]) {
        // Populated objects
  ...
  return {
    ...
    createdTime: new Date(groupMaster.createdAt).toISOString().split("T")[0],
    updatedTime: new Date(groupMaster.updatedAt).toISOString().split("T")[0],
    inactiveTime: new Date(groupMaster.updatedAt).toISOString().split("T")[0],
...
export const groupsMasterServices = {
  ...
  update: async (
    ...
    try {
      ...
      if (groupMasterData.groupName !== undefined) payload.groupName = groupMasterData.groupName;
      if (groupMasterData.groupModule !== undefined) payload.groupModule = groupMasterData.groupModule;
      if (groupMasterData.imei !== undefined) payload.imei = groupMasterData.imei;
  ...
  getGroupModules: async (): Promise<GroupModuleForDropdown[]> => {
    try {
      ...
      if (response.success) {
        return response.data.data.filter((groupModule) => groupModule.status === "active");
      } else {
