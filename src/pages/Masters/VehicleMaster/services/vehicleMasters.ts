...
const transformVehicleMasterToRow = (vehicleMaster: VehicleMasterData): Row => ({
  ...
  createdTime: new Date(vehicleMaster.createdAt).toISOString().split("T")[0],
  updatedTime: new Date(vehicleMaster.updatedAt).toISOString().split("T")[0],
  inactiveTime: new Date(vehicleMaster.updatedAt).toISOString().split("T")[0],
});
...
export const vehicleMasterServices = {
  getAll: async (
    ...
    try {
      const response: ApiResponse<VehicleMastersListResponse> = await getRequest(
  ...
  inactivate: async (id: string | number): Promise<{ message: string }> => {
    try {
      ...
      if (response.success) {
        return {
          message: response.message || "Vehicle Master inactivated successfully",
        };
      } else {
        throw new Error(response.message || "Failed to inactivate vehicle master");
      }
    } catch (error: any) {
  ...
  search: async (
    ...
    try {
      ...
      const response: ApiResponse<VehicleMastersListResponse> = await getRequest(
        `${urls.vehicleMastersViewPath}/search`,
        {
  ...
  getDriverModules: async (): Promise<DriverModule[]> => {
    try {
      const response: ApiResponse<{ data: DriverModule[] }> = await getRequest(
